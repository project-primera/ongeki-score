...
class ViewUserController extends Controller
{
    ...
    public function getUserPage($id, $mode = null){
        ...
        foreach ($score as $key => $value) {
            ...
            if(\App\UserInformation::IsPremiumPlan($user->id)){
                $score[$key]->ratingValue = sprintf("%.2f", OngekiUtility::RateValueFromTitle($score[$key]->title, $score[$key]->difficulty, $score[$key]->technical_high_score, $score[$key]->genre));
                $score[$key]->ratingValueRaw = $score[$key]->ratingValue;
                if (OngekiUtility::IsEstimatedRateValueFromTitle($score[$key]->title, $score[$key]->difficulty, $score[$key]->genre)) {
